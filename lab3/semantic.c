...
void print_func_error(struct node* node, struct table_item* func_item) {
    ...
    while (field_temp) {
        ...
        field_temp = field_temp->right_sibling;
    }
...
struct field* copy_field(struct field* src) {
    ...
    while (copy) {
        if (!head) {
            ...
            curr = head;
            copy = copy->right_sibling;
        } else {
            curr->right_sibling = new_field(copy->name, copy_type(copy->type));
...
struct sym_table* init_table() {
    ...
    // 添加read和write函数
    struct table_item* readFun = new_table_item(
        new_field(copy_str("read"),
                  new_type(FUNCTION, 0, NULL, new_type(BASIC, INT_TYPE))));

    struct table_item* writeFun = new_table_item(new_field(
        copy_str("write"),
        new_type(FUNCTION, 1, new_field("arg1", new_type(BASIC, INT_TYPE)),
                 new_type(BASIC, INT_TYPE))));

    add_table_item(table, readFun);
    add_table_item(table, writeFun);
...
struct table_item* get_table_item(struct sym_table* table, char* name) {
    ...
    while (item_head) {
        ...
        item_head = item_head->right_sibling;
    }
...
void add_table_item(struct sym_table* table, struct table_item* item) {
    ...
    item->right_sibling          = table->hash_array[hash_code];
...
void VarList(struct node* node, struct table_item* func_item) {
    ...
    while (lc->right_sibling) {
        ...
        if (paramdec) {
            curr->right_sibling = copy_field(paramdec);
            curr                = curr->right_sibling;
            argc++;
        }
    }
        ...
        if (struct_info != NULL) {
            ...
            while (struct_field) {
                if (!strcmp(payload->name, struct_field->name)) {
                    ...
                    last         = struct_field;
                    struct_field = struct_field->right_sibling;
                }
            }
            ...
            if (last == NULL) {
                ...
                last->right_sibling = copy_field(dec_item->field);
            }
        } else {
